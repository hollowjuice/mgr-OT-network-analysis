...
class ExtractFeatures1:
    ...
    def extract_features(self, df: pd.DataFrame) -> pd.DataFrame:
        df_features = df.copy()
        df_features = self._sanitize_input(df_features)
    ...
    def _sanitize_input(df: pd.DataFrame) -> pd.DataFrame:
        """
        Zamienia +inf/-inf w KOLUMNACH WEJŚCIOWYCH na najbardziej ekstremalną
        wartość skończoną zaobserwowaną w tej kolumnie (capping), zamiast na 0.
 
        Kontekst: jeśli standaryzacja Z-score (z rozdziału 2) liczy mu/sigma na
        fragmencie danych wolnym od ataków, a jakaś kolumna (np. status zaworu)
        ma tam wariancję zero, to sigma=0 i z=(x-mu)/sigma daje +-inf w momencie,
        gdy ta kolumna faktycznie się zmienia (najczęściej w trakcie ataku).
        Te wartości NIE są śmieciem - to realny sygnał ekstremalnego odchylenia,
        dlatego capujemy je do obserwowanego zakresu zamiast zerować (zerowanie
        ukryłoby tę anomalię zamiast tylko ustabilizować dalsze obliczenia).
        """
        ...
        if not affected.empty:
            print(
                f"[ExtractFeatures] UWAGA: {int(affected.sum())} wartości inf w "
                f"{len(affected)} kolumnach wejściowych (prawdopodobnie sigma=0 "
                "w oknie kalibracyjnym standaryzacji Z-score). Capuję do "
                "obserwowanego zakresu skończonego:"
            )
            for col, n in affected.items():
                finite_vals = df[col][np.isfinite(df[col])]
                cap_hi = finite_vals.max() if not finite_vals.empty else 0.0
                cap_lo = finite_vals.min() if not finite_vals.empty else 0.0
                df[col] = df[col].replace(np.inf, cap_hi).replace(-np.inf, cap_lo)
                print(f"  - {col}: {n} wartości -> capped to [{cap_lo:.4f}, {cap_hi:.4f}]")
